...
public class BiddingDataUtil extends BaseDataUtil {
	protected BiddingService biddingService = null;
	private String actionName=null;

	public BiddingDataUtil() {
		super();
		biddingService = (BiddingService) SpringUtils
				.getEntityService(Bidding.class);
	}
	...
	public Data1 getSpageDatas(Boolean hasImg) {
		...
		Bidding bidding = biddingService.findStaticSpageData(site
				.getId(), channel.getId());
		if (null == bidding)
			return null;
		data.setChannelName(channel.getChannelName());
		data.setPath(getPath(bidding.getId(), true));
		data.setContentList(getSpageDataContent(bidding));
		return data;
	}
	...
	private Data1 createData(int contentListSize, int imgListSize,boolean isContDetial) {
		Data1 data = new Data1();
		if (channel == null)
			return data;
	...
	private List<DataContent1> getSpageDataContent(Bidding bidding) {
		ArrayList<DataContent1> dataContentList = new ArrayList<DataContent1>();
		String path = getPath(bidding.getId(), false);
		DataContent1 dataContent1 = new DataContent1(bidding.getTitle(),
				null, path, "#",bidding.getCreateTime());
