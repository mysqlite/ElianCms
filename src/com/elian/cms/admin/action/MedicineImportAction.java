...
@Component
@Scope("prototype")
public class MedicineImportAction extends BaseAction {
	...
	public String save() {
		...
		List<Medicine> medicineList = medicineService.get(idList);
		...
		if (ElianUtils.CONTENT_SINGLE.equals(channel.getContentType())) {
			medicineList = medicineList.subList(0, 1);
		}

		Content temp = createContent();
		for (Medicine d : medicineList) {
	...
	private static final String INSTRUMENT = "medicine";
	...
	public void tree() {
		...
		if (AREA.equals(ApplicationUtils.getRequest().getParameter("type"))) {
			...
			addCompanyNode(companyList, list);
				...
				"type"))) {
			List<Medicine> medicineList = medicineService.findByAll(
					ApplicationUtils.getAjaxId(), null);
			addMedicineNode(medicineList, list);
		}
		else {
	...
	private void addMedicineNode(List<Medicine> medicineList,
	...
	private List<Area> loadAreaList() {
		...
		else if (ApplicationUtils.getSite().getComType().startsWith(
				ElianUtils.COMP_TYPE_COMP)) {
