...
@Component
@Scope("prototype")
public class HospitalImportAction extends BaseAction {
	...
	public String save() {
		...
		List<Hospital> hospitalList = hospitalService.get(idList);
		...
		if (ElianUtils.CONTENT_SINGLE.equals(channel.getContentType())) {
			hospitalList = hospitalList.subList(0, 1);
		}

		Content temp = createContent();
		for (Hospital d : hospitalList) {
			Content content = temp.clone();
			content.setEntityId(d.getId());
			content.setTitle(d.getHospName());
