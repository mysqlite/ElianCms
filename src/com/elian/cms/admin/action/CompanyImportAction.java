...
@Component
@Scope("prototype")
public class CompanyImportAction extends BaseAction {
	...
	public String save() {
		...
		List<Company> companyList = companyService.get(idList);
		...
		if (ElianUtils.CONTENT_SINGLE.equals(channel.getContentType())) {
			companyList = companyList.subList(0, 1);
		}
	...
	private static final String HOSPITAL = "company";
	...
	private List<Area> loadAreaList() {
		...
		if (ElianUtils.COMP_TYPE_MAIN.equals(ApplicationUtils.getSite()
			...
			List<Area> list = areaService
					.findAreaNameByAreaCode(ApplicationUtils.getCompany()
							.getArea().getId());
	...
	private void addCompanyNode(List<Company> companyList,
		...
		for (Company h : companyList) {
			list.add(createMapData(HOSPITAL, h.getId(), h.getName(),
							false));
		}
	}
