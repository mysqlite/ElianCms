...
<html lang="zh-CN">
	...
	<div class="main-top-hd clearfix">
	    <h3 class="cur">
	    <c:if test="${edit}">科室性质编辑</c:if>
	    <c:if test="${!edit}">科室性质添加</c:if>
	    </h3>
	</div>
<div class=body>
    <form id="hospTypeCOrU" name="hospTypeCOrU" method="post" action="${path}/admin/deptNature!save.action">
        ...
        <div class="divInputTable">
                   ...
                   <div class="listItem">
                      <label class="lbl-ipt-tit"><span class="star">*</span>类型类别：</label>
	                      <div class="select">
		                      ...
		                      <option value="${deptType.key}" <c:if test="${hospType.TypeClass eq deptType.key}">selected</c:if>>${deptType.value}</option>
		                      </select>
	                      </div>
         ...
         <div class="buttonArea">
            ...
            <input class="formButton" onclick="window.location='${path}/admin/deptNature!list.action'" value="返  回" type="button">
        </div>
        </form>
      </div>
   </body>
