 ...
 * Date: 2012-12-21
 *添加处理链接隐藏 @update Autor Gechuanyi
 */
(function($){
	...
	view = {
		...
		makeDOMNodeNameBefore: function(html, setting, node) {
			...
			/**此处被更改，主要处理链接隐藏 @update Autor Gechuanyi*/
			var onblur=" onBlur=this.href='javaScrpit:void(0);' ";
            var onmouseout=" onMouseOut=this.href='javaScrpit:void(0);' ";
            var thishref=(url!=null&&url.length>0)?" this.href="+"'"+url+"'"+(node.click!=null&&node.click.length>0?";"+node.click+";":""):"";
            var realhref=(url != null && url.length > 0&&setting.callback.onClick==null)?"href='javaScrpit:void(0);'":((url!=null&&url.length>0)?"href='"+url:"")+"'";
            if(url==null||setting.callback.onClick!=null){
                thishref=(node.click!=null&&node.click.length>0)?node.click+";":"";
                onblur="";
                onmouseout="";
            }
		    html.push('<a'+onblur+onmouseout);
            html.push(" id='", node.tId, consts.id.A, "' class='level", node.level,"' treeNode", consts.id.A," onclick=\"", (thishref|| thishref), 
                "\" ", realhref, " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
                "'");
